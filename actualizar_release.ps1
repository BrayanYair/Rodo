...
# Crea un nuevo release en GitHub y sube Rodo.exe
#
...
param(
    ...
    [string]$Token,
    [string]$Tag   = "v1.0.1",
    [string]$Notes = "- Nuevo icono de Rodo en el ejecutable"
)

$OWNER   = "BrayanYair"
$REPO    = "Rodo"
$EXE     = "C:\Users\Lenovo\Desktop\ProyectoAudio\rodolfo-amigo\dist\Rodo.exe"
$HEADERS = @{ Authorization = "token $Token"; Accept = "application/vnd.github.v3+json" }

if (-not (Test-Path $EXE)) {
    Write-Error "No se encontro Rodo.exe en $EXE"
    exit 1
}
...
try {
    ...
    Write-Host "      Release ya existe (ID: $($existing.id)), reemplazando Rodo.exe..."
    $oldAsset = $existing.assets | Where-Object { $_.name -eq "Rodo.exe" }
    if ($oldAsset) {
...
# 2. Subir Rodo.exe
Write-Host "[2/4] Subiendo Rodo.exe ($('{0:N1}' -f ((Get-Item $EXE).Length / 1MB)) MB)..."
$uploadUrl     = "https://uploads.github.com/repos/$OWNER/$REPO/releases/$releaseId/assets?name=Rodo.exe"
