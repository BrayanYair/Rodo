...
def parse_music(cmd: str):
    ...
    # Limpiar cola — regex flexible (elimina/borra/quita + toda/todas + cola/lista/canciones)
    if re.search(
    ...
    if any(w in cmd for w in [
        ...
        "quita las canciones de la cola", "vacia la lista",
    ]):
        return {"action": "clear_queue"}
...
def full_parse(raw_text: str, require_activator: bool = True):
    ...
    if cmd and len(cmd) > 1:
        greetings = ("hola", "buenas", "alo", "oye", "hey", "rodo", "rodolfo")
